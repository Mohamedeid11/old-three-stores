    ...
    <div class="kt-portlet kt-portlet--mobile">
        <div class="kt-portlet__head kt-portlet__head--lg">
			<div class="kt-portlet__head-label">
                ...
                <h3 class="kt-portlet__head-title">Expanses</h3>
            </div>
            <div class="kt-portlet__head-toolbar">
            	<div class="kt-portlet__head-wrapper">
                	<div class="kt-portlet__head-actions">
                	    @if(permission_checker(Auth::guard('admin')->user()->id, 'add_expanses'))
                    	<a href="#" class="btn btn-brand" data-toggle="modal" data-target="#NewExpanse">
                    	    <i class="la la-plus"></i> New Expanse
                    	</a>
                    	<div class="modal fade" id="NewExpanse" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    	<div class="modal-dialog">
                    	<div class="modal-content">
                    	<div class="modal-header">
                    	<h5 class="modal-title">New Expanse</h5>
                    	<button type="button" class="close" data-dismiss="modal" aria-label="Close">
	                    	<span aria-hidden="true">&times;</span>
                    	</button>
                    	</div>
                    	<div class="modal-body">
                        	<form class="kt-form" method="post" action="{{url('expanses')}}" enctype="multipart/form-data" id="new_expanse">
                				{{csrf_field()}}
                				...
                				<div class="form-group">
                					<div class="row">
                						<div class="col-md-12">	
                							...
                							<select class="form-control" required name="cat" id="new_expanses_selector" action="{{url('get_emps')}}">
                    	...
                    	@endif
                    	@if(permission_checker(Auth::guard('admin')->user()->id, 'delete_expanses'))
                    	...
                    	<div class="modal-body">
                    	<button type="button" class="btn btn-danger" id='delete_selected_orders' task="Delete" url="{{url('expanses/expanses_task')}}">Delete</button>
        ...
        <div class="kt-portlet__body">
			...
			<table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_2">
				...
				<th>Description</th>
				<th>Amount (EGP)</th>
				@if(permission_checker(Auth::guard('admin')->user()->id, 'edit_expanses') || permission_checker(Auth::guard('admin')->user()->id, 'delete_expanses'))
				<th>Action</th>
				@endif
			</tr>
			</thead>
			<tbody>
			    @foreach ($expanses as $exp)
    				...
    				<td>
            			<label class="kt-checkbox">
                			<input type="checkbox" class="check_single" name="category[]" value="{{$exp->id}}" />
                			<span></span>
            			</label>
        			</td>
        			@else
					...
					@if(permission_checker(Auth::guard('admin')->user()->id, 'edit_expanses') || permission_checker(Auth::guard('admin')->user()->id, 'add_expanses_category'))
					<td>
						<div class="dropdown">
							...
							<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
							    @if(permission_checker(Auth::guard('admin')->user()->id, 'edit_expanses'))
								<a class="dropdown-item" href="#" data-toggle="modal" data-target="#Expanse{{ $exp->id }}">Edit</a>
								@endif
								@if(permission_checker(Auth::guard('admin')->user()->id, 'delete_expanses'))
								<a class="dropdown-item" href="#" data-toggle="modal" data-target="#myModal-{{ $exp->id }}">Delete</a>
								@endif
							</div>
						</div>
                        @if(permission_checker(Auth::guard('admin')->user()->id, 'edit_expanses'))
                    	...
                    	<div class="modal-content">
                    	<div class="modal-header">
                    	<h5 class="modal-title">Edit Expanse</h5>
                    	<button type="button" class="close" data-dismiss="modal" aria-label="Close">
	                    	<span aria-hidden="true">&times;</span>
                    	</button>
                    	</div>
                    	<div class="modal-body">
                        	<form class="kt-form expanse_update_from" data-expanse="{{$exp->id}}" method="post" action="{{url('expanses/'.$exp->id)}}" enctype="multipart/form-data">
                				{{csrf_field()}}
                				...
                				<div class="form-group">
                					<div class="row">
                						<div class="col-md-12">	
                							<label>Category</label>												
                							<select class="form-control edit_cat_expanse" required name="cat" id="cat" num="{{$exp->id}}" action="{{url('get_emps')}}">
                							    <option value="" disabled selected>Choose Category</option>   
                							    @foreach ($cats as $cat)
						...
						<div class="modal fade" id="myModal-{{ $exp->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						    <div class="modal-dialog">
        						<div class="modal-content">
            						...
            						<div class="modal-body">
                						<form role="form" action="{{ url('/expanses/'.$exp->id) }}" class="" method="POST">
