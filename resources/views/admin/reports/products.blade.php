    ...
    <div class="kt-portlet kt-portlet--mobile">
        <div class="kt-portlet__head kt-portlet__head--lg">
			<div class="kt-portlet__head-label">
                ...
                <h3 class="kt-portlet__head-title">Products Reports</h3>
            </div>
        </div>
        <div class="kt-portlet__body">
				...
				<div class="form-group">
					<div class="row">
						...
						<div class="form-group col-md-12" id="order_number_gr">
							...
							<select class="form-control" name="order_tags[]" id="dashboard_order_tag" multiple>
								@foreach ($all_tags as $tag)
									<option value="{{$tag->id}}" @if(in_array($tag->id, $selected_order_tags)) selected @endif>{{$tag->title}}</option>
								@endforeach
							</select>
						</div>
						<div class="col-md-5 col-6">
							<div class="form-group">
								...
								<input type="text" class="form-control" name="tags" id="product_tags" value="{{implode(',', $selected_tags)}}" />	
							</div>
						</div>
            ...
            <div id="all_report_products" class="notloaded dashboard_results" data-url="{{url('report_products_table')}}"></div>
...
function get_product_table()
{
    ...
    $.ajax({
        ...
        data: {from_date: '{{$from_date}}', to_date: '{{$to_date}}', city: '{{implode(',', $selected_city)}}', rep:'{{implode(',', $selected_rep)}}', moderator:'{{implode(',', $selected_moderator)}}', 
			...
			order_tags: '{{ implode(',', $selected_order_tags) }}'
        },
        url: action,
...
$(document).ready(function() { 
	...
	function products_filter_with_tags()
	{
		...
		var action = "{{url('product_tag_serch')}}";
		$.ajax({
			type: 'POST',
			data: {tags: tags, products: products},
			url: action,
			success: function(data) 
			{
				$('#dashboard_product_item').html(data);
			}
		});
	}
});
