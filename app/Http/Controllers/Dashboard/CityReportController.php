...
class CityReportController extends Controller
{
    public function index()
    {
        ...
        return view('admin.reports.cities')->with(['first_order'=>$first_order, 'from_date'=>$from_date, 'to_date'=>$to_date, 'selected_city'=>$selected_city, 'cities'=>$cities, 
        ...
        'selected_client_type'=>$selected_client_type, 'selected_tags' => $selected_tags, 'all_tags'=>$all_tags]);
    }

    public function report_city_table (Request $request)
    {
        ...
        foreach ($all_orders as $item)
        {
            ...
            $itx = "CITY - ".$item->order_info->city;
            if(in_array($itx, $items))
            {
                $in = array_search($itx, $items);
                $cities[$in][0] = $cities[$in][0] + $all_qty;
                $total_items = $total_items + $all_qty;

                if($order->status == 5 || $order->status == 6) {$cities[$in][2] = $cities[$in][2] + $all_qty;}
                else if($order->status == 7 || $order->status == 8) {$cities[$in][4] = $cities[$in][4] + $all_qty;}
                else {$cities[$in][3] = $cities[$in][3] + $all_qty;}
                ...
                $cities[$in][5] = $cities[$in][5] + $amount;
            }
            else
            {
                ...
                $items[] = $itx;
                $px = "";
                $px = $item->order_info->city_info->title;
                $cost = 0;
                $cities[] = array($all_qty, $px, $won, $open, $loss, $amount, $cost);
                $total_items = $total_items + $all_qty;
            }
        }
        rsort($cities);
                ...
                <tbody>
                    ...
                    for ($i = 0; $i < count($cities); $i++)
                    {
                        ...
                        <tr>
                            ...
                            <td><?php echo number_format(($cities[$i][0] / $total_items) * 100, 2); ?> %</td>
                            <td><?php echo number_format($cities[$i][2], 2); ?></td>
                            <td><?php echo number_format($cities[$i][3], 2); ?></td>
                            <td><?php echo number_format($cities[$i][4], 2); ?></td>
                            <td><?php echo number_format($cities[$i][5], 2); ?></td>
                        </tr>
                        <?php
                    }
                    ?>
