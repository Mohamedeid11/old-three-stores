...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        else
        {
            ...
            if(permission_group_checker(Auth::guard('admin')->user()->id, 'Selling Orders'))
            {
                ...
                {
                    $sell_orders = SellOrder::where('hide', '=', 0)->get();
                }
                else
                {
                   $sell_orders = SellOrder::where('hide', '=', 0)->whereIn('status', [5, 7])->where('delivered_by', '=', Auth::guard('admin')->user()->id)->get(); 
                }
            }
            ...
            $buy_orders = BuyOrder::where('hide', '=', 0)->get();
            return view('admin.home')->with(['admins'=>$admins, 'clients'=>$clients, 'agents'=>$agents, 'products'=>$products, 'sell_orders'=>$sell_orders, 
            'buy_orders'=>$buy_orders, 'products'=>$products]);
        }
    }
    ...
    {
        $order = SellOrder::where('order_number', '=', $request->search)->where('hide', '=', 0)->first();
        if($order !== NULL)
        {
            ?>
...

    {
        ...
        $orders = SellOrder::where('hide', '=', 0)->whereIn('status', [0, 1, 11])->orderBy('id', 'desc')->get();
...

        {
...

            {
                ...
                $xch = Fulfillment::where('order', '=', $order->id)->where('item', '=', $item->id)->first();

                if($xch !== NULL)

                {

                    $xxch = Fulfillment::where('order', '=', $order->id)->where('item', '=', $item->id)->get()->count();
...

                    {
...

                        {
                            ...
                            $products[$in][0] = $products[$in][0] + ($item->qty - $xxch);
...

                    {
                        ...
                        if($item->qty > 0) {$zxxch = $item->qty - $xxch;} else {$zxxch = $item->qty + $xxch;}

                        $products[] = array($zxxch, $px);       
...

                {
...

                    {
                        ...
                        $products[$in][0] = $products[$in][0] + $item->qty;
        ...
        sort($products_name, SORT_NATURAL | SORT_FLAG_CASE);



        return view('admin.print_purchase')->with(['products'=>$products, 'products_name'=>$products_name, 'products_namex'=>$products_namex]);
    ...
    public function product_dashboard_options(Request $request)
    {
            ...
            <tbody>
                    ...
                    foreach ($colors as $color)
                    {
                        if($sizes->count() == 0)
                        {
                            ...
                            <tr>
                                ...
                                <td><?php echo $this->product_available_units_withoutpat($product, $color->color_info->id, 0); ?></td>
                        ...
                        else
                        {
                            foreach ($sizes as $size)
                            {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $this->product_available_units_withoutpat($product, $color->color_info->id, $size->size_info->id); ?></td>
                ...
                else if($colors->count() == 0 && $sizes->count() > 0)
                {
                    foreach ($sizes as $size)
                    {
                        ...
                        <tr>
                            <td><?php echo $size->size_info->title; ?></td>
                            <td><?php echo $this->product_available_units_withoutpat($product, 0, $size->size_info->id); ?></td>
        ...
        else
        {
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    <tr>
                        <td><?php echo $this->product_available_units_withoutpat($product, 0, 0); ?></td>
