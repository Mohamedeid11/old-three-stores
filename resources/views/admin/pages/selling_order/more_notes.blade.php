...
<div class="table-responsive">
	<table class="table table-striped- table-bordered table-hover table-checkable mb-0 table-info">
		<tbody>
			@foreach ($all_notes as $note)
				
				@if ($note->order_info->hide == 0 && $loop->index > 0)
				<tr id="note_row_{{$note->id}}" @if($note->status == 1) class="completed_note" @endif>
					...
					<td>{{$note->order_info->order_number}}</td>
					<td>{{$note->order_info->status_info->title}}</td>
					<td>
						{{$note->note}}
					...
					<td>{{$note->order_info->client_info->name}}</td>
					
					<td>@if($note->admin_info) {{$note->admin_info->name}} @endif</td>
					<td>@foreach($note->reps as $ss) @if($loop->iteration > 1) - @endif {{$ss->rep_info->name}} @endforeach</td>
				...
				@else
					<td id="load_order_notes_{{$note->order}}" colspan="10" class="hide load_order_notes p-0"></td>
				@endif
...
@else
<p class="text-center font-weight-bold text-danger mb-2 mt-2">No More Notes</p>
@endif